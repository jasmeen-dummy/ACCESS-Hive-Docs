...
`base_configuration` :octicons-check-circle-fill-16:
...
- [x] `contact` (1)
    ...
    1.  Email address or other contact details for who to contact regarding this data.<br>
        examples: <ul><li>access.nri@anu.edu.au</li><li>John Smith (john.smith@email.com)</li></ul> 

- [x] `conventions` (1)
    { .annotate }

    1.  Convention(s) used with their versions, as a comma separated list <br>
        example: CF-1.11,ACDD-1.3

- [x] `data_specification` (1)
    { .annotate }

    1.  Version of this data specification used to generate the data.<br>
        example: ACCESS Output Data Specification v2-0-0 XX.XXXX/zenodo.XXXXX

- [x] `date_created` (1)
    { .annotate }

    1.  Date and time the file was created. Follow ISO 8601, i.e. 'YYYY-MM-DDTHH:MM:SSZ' <br>
        example: 2025-10-07T11:10:00Z <br>
        rules: Must match regex: ^\d{4}-(1[012]\|0[1-9])-(3[01]\|[12][0-9]\|0[1-9])T([01][0-9]\|2[0-3]):[0-5][0-9]:[0-5][0-9]Z$

- [ ] `date_metadata_modified` (1)
    { .annotate }

    1.  Date and time the metadata for this file was last modified. Note that this applied just to the metadata, not the data. Follow ISO 8601, i.e. 'YYYY-MM-DDTHH:MM:SSZ'<br>
        example: 2025-10-07T11:10:00Z <br>
        rules:  Must match regex: ^\d{4}-(1[012]\|0[1-9])-(3[01]\|[12][0-9]\|0[1-9])T([01][0-9]\|2[0-3]):[0-5][0-9]:[0-5][0-9]Z$
...
- [ ] `date_modified` (1)
    ...
    1.  Date and time the data for this file was last modified. Note that this applied just to the data, not the metadata. Follow ISO 8601, i.e. 'YYYY-MM-DDTHH:MM:SSZ' (1) Rule(2)
...
- [x] `experiment_repo` (1)
    ...
    1.  Git repository URL that describes the experiment<br>
        example: https://github.com/ACCESS-Community-Hub/access-esm1.6-dev-experiments    

- [x]`experiment_uuid` (1) 
    { .annotate }

    1.  The experiment UUID generated by Payu. Note: this may be the same as id.<br>
        example: 698E600B-BECF-4CBA-994F-A663A22FCDDF

- [x] `frequency` (1)
    { .annotate }

    1.  Sampling frequency of the data<br>
        examples: <ul><li>12hr</li><li>1day</li><li>1yr</li></ul>
        rules: Must match one of these regex: <ul><li>\^fx\$</li><li>\^subhr\$</li><li>\^\d+min\$</li><li>\^\d+hr\$</li><li>\^\d+day\$</li><li>\^\d+mon\$</li><li>\^\d+yr\$</li><li>\^\d+dec\$</li></ul>  

- [ ] `grid` (1)
    {  .annotate }
    1.  Brief description of output grid characteristics or reference to grid specification. Should be included if the grid is not defined by the dimensions in the file.

- [x] `license` (1)
    { .annotate }

    1.  Information on the license for the data to ensure all users have access to the terms of use. Use SPDX license identifiers where possible.
        The default license for ACCESS-NRI is CC-BY-4.0, users should change as needed.<br>
        example: CC-BY-4.0

- [x] `model` (1)
    { .annotate }

    1.  Name of the model used to create the data.<br>
        example: ACCESS-ESM1.6 

- [x] `model_version` (1)
    { .annotate }

    1.  Version of the model used to create the data. Please note here if the model has been modified from an official release, ideally with links to the changes. <br>
        example: <ul><li>2025.06.001</li><li>2025.06.001 (modified by John Smith, [link to repo/paper describing modifications])</li></ul> 

- [x] `realm` (1)
    { .annotate }

    1.  Realm where the data variable is defined. <br>
        examples: <ul><li>atmos</li><li>landIce</li></ul>
        rules: Must be one of the following: <ul><li>aerosol</li><li>atmos</li><li>atmosChem</li><li>land</li><li>landIce</li><li>none</li><li>ocean</li><li>ocnBgchem</li><li>seaIce</li><li>unknown</li></ul>

- [x] `run_id` (1)
    { .annotate }

    1.  Git hash for the commit associated with the experiment run<br> 
        examples: <ul><li>3a38fe4</li><li>3a38fe435e156700ab649c4921e99f7c68a168bc</li></ul>
...
1.  <ul><li>tas</li><li>huss</li><li>uas,vas</li></ul> 
